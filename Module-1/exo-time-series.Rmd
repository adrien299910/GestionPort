...
```{r, echo=TRUE}
ts.air <- get.ts(folder="CAC40", ticker="air.pa")
```

```{r, fig-1, echo=FALSE, fig.width=10, fig.cap="Zodiac Aerospace"}
plot(ts.air, main='Rendement quotidien - Air Liquide')
```
...
Calculer la matrice de corrélation des rendements quotidients des actions de l'Eurostoxx50echerchez une manière synthétique de présenter le résultat.
```{r, echo=TRUE, cache=TRUE}
ts.EuroStoxx50 <- get.all.ts(folder="EuroStoxx50", returns=TRUE, combine=TRUE)

library(corrplot) 
mcor <- cor(ts.EuroStoxx50)
...
good.df
```
```{r, best_return, echo=TRUE, cache=TRUE}
sorted_df <- good.df[order(-good.df$ret),]
best_ret <-  head(sorted_df, 10)
best_ret
```
```{r, worst_return, echo=TRUE, cache=TRUE}
best_ret 
worst_ret <-tail(sorted_df,10)
worst_ret
```
Déterminez les 10 titres avec les meilleurs rendements moyens, et les 10 titres avec les rendements les plus bas.
